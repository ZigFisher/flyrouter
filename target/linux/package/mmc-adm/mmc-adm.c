...
MODULE_AUTHOR("Madsuk/Rohde/Vlad");
MODULE_DESCRIPTION("Driver MMC/SD-Cards");
MODULE_SUPPORTED_DEVICE("Devices based on ADM5120 CPU");
MODULE_LICENSE("GPL");


//#define DPRINTF printk - For debug only !
#define DPRINTF if(0)printk

//                     ----------------
//                    / 1 2 3 4 5 6 7 x| SD/MMC Card
//                    |x               |

// Pin connection definition
#define GPIO_DI		15  // led speed eth2  (on SD/MMC card pin 2 - DI)
#define GPIO_DO		21  // led speed eth4  (on SD/MMC card pin 7 - DO)
#define GPIO_CLK	18  // led speed eth3  (on SD/MMC card pin 5 - CLK)
#define GPIO_CS		12  // led speed eth1  (on SD/MMC card pin 1 - CS)
...
static void adm5120_gpio_set(int id, int state)
{
		...
		DPRINTF("mmc.c setting gpio%d: to %d\n", id, state);
    	if(id <= 7 && id >= 0) {
			if (state) {
...
static int adm5120_gpio_get(int id)
{
		...
		DPRINTF("mmc.c reading gpio%d: ", id);
    	if(id <= 7 && id >= 0) {
...
{
	adm5120_gpio_set(GPIO_CS, 0);
}

static void mmc_spi_cs_high(void)
{
	adm5120_gpio_set(GPIO_CS, 1);
}
...
static unsigned char mmc_spi_io(unsigned char data_out)
{
  ...
  for(i=0; i<8; i++) {
    if(data_out & (0x01 << (7-i)))
		adm5120_gpio_set(GPIO_DI, 1);
    else
		adm5120_gpio_set(GPIO_DI, 0);
    
	adm5120_gpio_set(GPIO_CLK, 1);
    
	tmp_data = adm5120_gpio_get(GPIO_DO);
    
	adm5120_gpio_set(GPIO_CLK, 0);
    result <<= 1;
